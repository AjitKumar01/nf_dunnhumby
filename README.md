...
| **`NESTED_MODEL.md`** | the nested model that puts back what the flat one dropped: category incidence with an estimated nesting coefficient, quantity as counts, and stores. **all 8 acceptance criteria met** | you want the price decomposition and basket generation |
