...
class BaselineExactness(unittest.TestCase):
    def test_bernoulli_is_exactly_truncated_by_size_and_category(self):
        ...
        self.assertAlmostEqual(float(m.loglik(d, nmax=2, category_cap=2)),
                               -math.log(10), places=12)
        self.assertAlmostEqual(float(m.loglik(d, nmax=2, category_cap=1)),
    ...
    def test_multinomial_respects_category_cap(self):
        ...
        zero_index(m.idx)
        d = batch([0, 1, 2, 3], [0, 0, 0, 1], [0, 3])
        self.assertAlmostEqual(float(m.loglik(d, category_cap=1)), -math.log(3), places=12)
        self.assertAlmostEqual(float(m.loglik(d, category_cap=None)), -math.log(6), places=12)
    ...
    def test_ndpp_is_exactly_conditioned_nonempty(self):
        ...
        self.assertAlmostEqual(float(m.loglik(d)), -math.log(15), places=12)
    ...
    def test_shopper_exact_set_sum_and_nonempty_conditioning(self):
        ...
        self.assertAlmostEqual(float(got), -math.log(9), places=12)
    ...
    def test_shopper_forces_checkout_at_maximum_size(self):
        ...
        self.assertAlmostEqual(float(got), math.log(2 / 9), places=12)
