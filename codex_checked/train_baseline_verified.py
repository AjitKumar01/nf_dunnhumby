...
def main(a):
    ...
    path = os.path.join(OUT, f"v3_bl_verified_{a.model}{suffix}.pt")
    best_path = os.path.join(OUT, f"v3_bl_verified_{a.model}{suffix}_best.pt")
    ...
    for iteration in range(start + 1, a.iters + 1):
        ...
        if iteration % a.eval_every == 0 or iteration == a.iters:
            ...
            log(f"it {iteration:6d}/{a.iters} loss {float(loss):9.3f} "
    ...
    with open(os.path.join(OUT, f"v3_bl_verified_{a.model}{suffix}.json"), "w") as stream:
