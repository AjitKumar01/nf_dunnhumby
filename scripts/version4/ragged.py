...
def poly_tree_degree_aware(P, degree, nmax, buckets=(0, 1, 2, 4, 8, 16, 32, 64)):
    """Reference degree-aware product of per-category generating polynomials.

    ``P`` has shape ``[D, B, C, N+1]`` and ``degree`` has shape ``[B, C]``.
    Coefficients above ``degree[b,c]`` must be algebraic zeros.  The ordinary tree still
    multiplies all ``N+1`` entries at every internal node; this version groups pairs by a
    power-of-two degree envelope and never presents the proven zero suffix to the fused
    polynomial multiplier.  The output remains dense and has exactly the same support.

    This PyTorch implementation is deliberately a correctness/complexity prototype.  Its
    grouped ``index_copy`` calls are not assumed to be faster than the dense tree; only an
    end-to-end frozen benchmark may authorize replacing the production kernel.  A native
    fused forward/reverse operator can consume the same degree contract if launch overhead
    prevents this reference form from reaching the complexity bound.
    """
    if P.ndim != 4 or degree.shape != P.shape[1:3]:
        raise ValueError("degree-aware tree needs P[D,B,C,N+1] and degree[B,C]")
    nmax = int(nmax)
    if P.shape[-1] < nmax + 1:
        raise ValueError("polynomial axis is shorter than nmax+1")
    degree = degree.to(dtype=torch.long, device=P.device).clamp(0, nmax)
    limits = sorted({int(x) for x in buckets if 0 <= int(x) < nmax} | {nmax})
    ...
    while P.shape[-2] > 1:
        D, B, C, _ = P.shape
        if C % 2:
            ident = torch.zeros(D, B, 1, nmax + 1, dtype=P.dtype, device=P.device)
            ident[..., 0] = 1.0
            P = torch.cat([P, ident], dim=2)
            degree = torch.cat([
                degree, torch.zeros(B, 1, dtype=torch.long, device=P.device)], dim=1)
            C += 1
        A, G = P[:, :, 0::2], P[:, :, 1::2]
        da, dg = degree[:, 0::2], degree[:, 1::2]
        pairs = C // 2
        out = torch.zeros(D, B * pairs, nmax + 1, dtype=P.dtype, device=P.device)

        previous_a = -1
        for limit_a in limits:
            select_a = (da > previous_a) & (da <= limit_a)
            previous_g = -1
            for limit_g in limits:
                select = select_a & (dg > previous_g) & (dg <= limit_g)
                previous_g = limit_g
                where = torch.nonzero(select, as_tuple=False)
                if where.numel() == 0:
                    continue
                bi, ci = where[:, 0], where[:, 1]
                product = _poly_mul_trunc(
                    A[:, bi, ci, :limit_a + 1],
                    G[:, bi, ci, :limit_g + 1], nmax)
                if product.shape[-1] < nmax + 1:
                    product = torch.nn.functional.pad(
                        product, (0, nmax + 1 - product.shape[-1]))
                out = out.index_copy(1, bi * pairs + ci, product)
            previous_a = limit_a
        P = out.view(D, B, pairs, nmax + 1)
        degree = (da + dg).clamp(max=nmax)
    return P[..., 0, :]
...
def anisotropic_gh_grid(orders):
    """Positive tensor Gauss--Hermite rule with a declared order per coordinate."""
    import itertools
    orders = tuple(int(order) for order in orders)
    if not orders or any(order < 1 for order in orders):
        raise ValueError("Gauss--Hermite orders must all be positive")
    rules = []
    for order in orders:
        nodes, weights = np.polynomial.hermite_e.hermegauss(order)
        rules.append((nodes, weights / math.sqrt(2 * math.pi)))
    shape = [range(order) for order in orders]
    node_rows, weight_rows = [], []
    for index in itertools.product(*shape):
        node_rows.append([rules[k][0][index[k]] for k in range(len(orders))])
        weight_rows.append(float(np.prod(
            [rules[k][1][index[k]] for k in range(len(orders))])))
    return (torch.as_tensor(np.asarray(node_rows), dtype=torch.float64),
            torch.as_tensor(np.asarray(weight_rows), dtype=torch.float64))


def sobol_grid(d, n, seed=0, replicates=1, antithetic=False):
    ...
    for r in range(replicates):
        ...
        if antithetic:
            if per < 4 or (per // 2) & (per // 2 - 1):
                raise ValueError("antithetic nodes per scramble must be twice a power of two")
            half = e.draw(per // 2).double()
            blocks.append(torch.cat([half, 1.0 - half], dim=0))
        else:
...
def sobol_mixture_grid(d, n, seed=0, replicates=1, components=2, antithetic=False):
    ...
    for r in range(replicates):
        for c in range(components):
            ...
            if antithetic:
                if per < 4 or (per // 2) & (per // 2 - 1):
                    raise ValueError(
                        "antithetic nodes per mixture component must be twice a power of two")
                half = e.draw(per // 2).double()
                blocks[r, c] = torch.cat([half, 1.0 - half], dim=0)
            else:
...
def set_quad(model, quad_q=0, qmc_n=0, qmc_seed=0, Kz=None, probe=0,
             ...
             mode_logtol=8.0, mode_sep=1.0, mix_n=0, modes=2,
             antithetic=False, subspace_rank=0, subspace_iters=0,
             subspace_eps=0.05):
    ...
    if qmc_n > 0:
        model.quad_a = sobol_grid(d, int(qmc_n), seed=int(qmc_seed),
                                  replicates=int(qmc_reps), antithetic=bool(antithetic))
        ...
        model.quad_max_modes = int(modes)
        model.quad_subspace_rank = int(subspace_rank)
        model.quad_subspace_iters = int(subspace_iters)
        model.quad_subspace_eps = float(subspace_eps)
        if (model.quad_subspace_rank < 0 or model.quad_subspace_rank > d
                or model.quad_subspace_iters < 0 or model.quad_subspace_eps <= 0):
            raise ValueError("invalid QMC covariance-subspace configuration")
        if model.quad_max_modes < 2:
            raise ValueError("size-stratified QMC needs at least two proposal modes")
        ...
        if model.quad_size_bands:
            model.quad_mix_a = sobol_mixture_grid(
                ...
                replicates=int(qmc_reps), components=model.quad_max_modes,
                antithetic=bool(antithetic))
            return (f"size-stratified multimode scrambled-Sobol, Kz={d}, "
                    f"{int(qmc_n)} one-mode/{model.quad_mix_n} up-to-"
                    f"{model.quad_max_modes}-mode nodes in "
                    f"{int(qmc_reps)} scrambles, seed {int(qmc_seed)}, "
                    f"{int(size_steps)} vectorised size-mode steps, "
                    f"retained modes within {float(mode_logtol):g} nats and "
                    f"separation {float(mode_sep):g}"
                    + (f", rank-{int(subspace_rank)} covariance frame with "
                       f"{int(subspace_iters)} Krylov steps"
                       if int(subspace_rank) else ", unit covariance frame")
...
class RaggedModel(torch.nn.Module):
    ...
    def __init__(self, J, N, C, K=8, Kz=3, nmax=24, R=4, seed=0,
        ...
        self.quad_max_modes = 2
        self.quad_mix_n = 0
        self._last_qmc_logz_se = None
        self._last_qmc_mode_count = None
        self._last_qmc_mode_weights = None
        self._last_qmc_mode_gap = None
        self._last_qmc_mode_sep = None
        self._last_qmc_curv_min = None
        self._last_qmc_scale_max = None
        self._last_qmc_en_se = None
        self._last_qmc_nested_logz_gap = None
        self._last_qmc_nested_en_gap = None
        # Optional local-covariance sketch used only by an explicitly configured hard-trip
        # retry.  The ordinary rule leaves this at zero and pays no curvature cost.
        self.quad_subspace_rank = 0
        self.quad_subspace_iters = 0
        self.quad_subspace_eps = 0.05
    ...
    def _size_multimode_centres(self, ix, drop_empty, cache):
        ...
        # Select distinct basins in descending target height.  The original implementation
        # screened all seven candidates and then retained only two.  That is not a valid
        # variance argument: three (or more) size phases can coexist, and Owen-replicate
        # spread cannot reveal a basin absent from every replicate.  Keep up to the fixed
        # proposal budget instead.  Repeating the top centre in unused slots preserves a
        # rectangular tensor; in the exact balance denominator those repeated Gaussian
        # components simply give the dominant basin more proposal mass.
        ...
        max_modes = min(int(getattr(self, "quad_max_modes", 2)), M)
        selected = top[:, None, :].expand(-1, max_modes, -1).clone()
        allocation = torch.zeros(B, max_modes, dtype=z.dtype, device=z.device)
        counts = torch.ones(B, dtype=torch.long, device=z.device)
        second_gap = torch.full_like(top_score, torch.inf)
        second_dist = torch.zeros_like(top_score)
        with torch.no_grad():
            for b in range(B):
                chosen = [int(top_id[b])]
                for candidate in torch.argsort(scores[b], descending=True).tolist():
                    if candidate == chosen[0]:
                        continue
                    if float(scores[b, candidate]) < (
                            float(top_score[b]) - float(self.quad_mode_logtol)):
                        break
                    distance = torch.stack([
                        (z[b, candidate] - z[b, old]).norm() for old in chosen]).amin()
                    if float(distance) < float(self.quad_mode_sep):
                        continue
                    chosen.append(candidate)
                    if len(chosen) == max_modes:
                        break
                counts[b] = len(chosen)
                for slot, candidate in enumerate(chosen):
                    selected[b, slot] = z[b, candidate]
                # Detached height weights are an importance-proposal choice, not a model
                # factor.  They make the proposal exact for separated equal-curvature
                # phases.  If the rectangular tensor needs filler components, split the
                # dominant phase's weight over its identical copies so q itself is
                # unchanged.
                unique_weight = torch.softmax(scores[b, chosen], dim=0)
                # Every rectangular component slot carries the same number of Sobol
                # points.  Filling every spare slot with the top phase allocated 3:1
                # samples even when two phases each held half the mass—the exact
                # high/low-size coexistence where E[n] is most sensitive.  Allocate the
                # integer slots approximately in proportion to detached phase mass while
                # retaining at least one slot per selected phase.  Splitting a phase's
                # alpha across identical copies leaves q and the balance estimator exactly
                # unchanged; it only spends the fixed node budget where variance lives.
                slot_count = torch.ones(len(chosen), dtype=torch.long, device=z.device)
                for _ in range(max_modes - len(chosen)):
                    add_to = int(torch.argmax(unique_weight / slot_count).item())
                    slot_count[add_to] += 1
                slot = 0
                for phase, candidate in enumerate(chosen):
                    copies = int(slot_count[phase])
                    for _ in range(copies):
                        selected[b, slot] = z[b, candidate]
                        allocation[b, slot] = unique_weight[phase] / copies
                        slot += 1
                if len(chosen) > 1:
                    second_gap[b] = top_score[b] - scores[b, chosen[1]]
                    second_dist[b] = (z[b, chosen[1]] - top[b]).norm()

        self._last_qmc_mode_count = counts.detach()
        self._last_qmc_mode_weights = allocation.detach()
        self._last_qmc_mode_gap = second_gap.detach()
        self._last_qmc_mode_sep = second_dist.detach()
        return (selected.detach(), allocation.detach(), (counts > 1).detach(),
                top.detach())

    def _size_subspace_frame(self, ix, centres, cache, drop_empty):
        """Detached low-rank local Laplace frame from first-gradient Hessian products.

        The exact latent Hessian is ``Cov(mu_S|z,x)-I``.  ``esp_bucketed`` has a verified
        first backward but no valid double backward, so central differences of that first
        gradient supply products with ``Cov(mu_S|z,x)``.  A block subspace step is optional;
        Rayleigh--Ritz then diagonalises the measured covariance inside the local subspace.
        Directions not represented by the sketch retain unit variance.
        """
        B, M, Kz = centres.shape
        rank = min(int(self.quad_subspace_rank), Kz)
        if rank < 1:
            raise ValueError("quad_subspace_rank must be positive in subspace frame")
        nit = int(self.quad_subspace_iters)
        if nit < 0 or self.quad_subspace_eps <= 0:
            raise ValueError("subspace iterations must be nonnegative and epsilon positive")
        with torch.no_grad():
            present = torch.unique(ix.item)
            phi_u = self.phi.index_select(0, present).detach()
            gram = phi_u.transpose(0, 1) @ phi_u
            evals, q0 = torch.linalg.eigh(gram)
            q0 = q0.index_select(1, torch.argsort(evals, descending=True))
            basis = q0[:, :rank].transpose(0, 1).view(1, 1, rank, Kz)
            basis = basis.expand(B, M, -1, -1).clone()
        C_mode = {k: (v.detach() if torch.is_tensor(v) else v)
                  for k, v in cache.items()}
        eps = float(self.quad_subspace_eps)

        def covariance_products(direction):
            P = direction.shape[2]
            plus = centres[:, :, None, :] + eps * direction
            minus = centres[:, :, None, :] - eps * direction
            z = torch.cat([plus, minus], dim=2).reshape(B, M * 2 * P, Kz)
            zz = z.detach().requires_grad_(True)
            with torch.enable_grad():
                target = (log_f_sparse(self, zz, ix, C_mode, drop_empty,
                                       detach_params=True)
                          - 0.5 * zz.square().sum(-1))
                grad = torch.autograd.grad(target.sum(), zz)[0].detach()
            grad = grad.reshape(B, M, 2 * P, Kz)
            hv = (grad[:, :, :P] - grad[:, :, P:]) / (2.0 * eps)
            return hv + direction                 # (H + I)v = Cov(mu|z,x)v

        for _ in range(nit):
            cb = covariance_products(basis)
            q, _ = torch.linalg.qr(cb.transpose(-1, -2), mode="reduced")
            basis = q.transpose(-1, -2)

        cb = covariance_products(basis)
        projected = torch.matmul(basis, cb.transpose(-1, -2))
        projected = 0.5 * (projected + projected.transpose(-1, -2))
        lam, rotate = torch.linalg.eigh(projected)
        order = torch.argsort(lam, dim=-1, descending=True)
        lam = torch.gather(lam, -1, order).clamp_min(0.0)
        rotate = torch.gather(rotate, -1, order.unsqueeze(-2).expand_as(rotate))
        leading = torch.matmul(basis.transpose(-1, -2), rotate)
        Q = torch.empty(B, M, Kz, Kz, dtype=centres.dtype, device=centres.device)
        with torch.no_grad():
            for b in range(B):
                for m in range(M):
                    # The unresolved complement has unit scale, so its orientation does
                    # not change the continuous proposal density.  It matters greatly to
                    # a finite Sobol block: retain the catalogue-active ordering there
                    # instead of accepting LAPACK's arbitrary null-space completion.
                    seeded = torch.cat([leading[b, m], q0], dim=1)
                    Q[b, m] = torch.linalg.qr(seeded, mode="reduced")[0]
        raw_curvature = 1.0 - lam
        # Rayleigh--Ritz is evaluated at every screened centre.  Some size-screen centres
        # are deliberate bridges between phases rather than local maxima.  A non-positive
        # precision there is NOT a Laplace covariance and must not be inverted: the old
        # clamp turned it into sd=8 and made a few tail nodes dominate every retry.  The
        # exact target is a mixture of unit-covariance Gaussians and the separated proposal
        # centres already cover its phases, so bridge centres retain the safe unit scale.
        # At genuine positive-curvature centres use local widening, capped at sd=2; all
        # proposal eigenvalues remain >=1/2 (in fact >=1), preserving finite second moment.
        curvature = raw_curvature.clamp(0.25, 1.0)
        curvature = torch.where(raw_curvature > 0.0, curvature,
                                torch.ones_like(curvature))
        sd = torch.ones(B, M, Kz, dtype=centres.dtype, device=centres.device)
        sd[:, :, :rank] = curvature.rsqrt()
        self._last_qmc_curv_min = raw_curvature.amin(dim=(1, 2)).detach()
    ...
    def _size_proposal_frame(self, ix, centres, cache, drop_empty):
        ...
        """
        if int(getattr(self, "quad_subspace_rank", 0)) > 0:
            return self._size_subspace_frame(ix, centres, cache, drop_empty)
        B, M, Kz = centres.shape
        with torch.no_grad():
            present = torch.unique(ix.item)
            phi_u = self.phi.index_select(0, present).detach()
    ...
    def _size_multimode_proposal(self, ix, drop_empty, cache):
        ...
        centres, allocation, has_multiple, top = self._size_multimode_centres(
            ix, drop_empty, cache)
        B, Kz = ix.B, self.Kz
        if bool(has_multiple.any()):
            ...
            self._last_qmc_layout = (reps, M, per)
            scaled = x.unsqueeze(0) * sd[:, None, :, None, :]
            delta = (torch.einsum("brmlk,bmdk->brmld", scaled, Q)
                     if Q.ndim == 4 else torch.matmul(scaled, Q.transpose(0, 1)))
            ...
            eigcoord = (torch.einsum("bdmj,bmjk->bdmk", diff, Q)
                        if Q.ndim == 4 else torch.matmul(diff, Q))
            comp_lq = (-0.5 * (eigcoord / sd[:, None, :, :]).square().sum(-1)
                       - sd.log().sum(-1)[:, None, :]
                       - 0.5 * Kz * math.log(2.0 * math.pi))
            logq = torch.logsumexp(
                comp_lq + allocation[:, None, :].log(), dim=-1)
            logprior = (-0.5 * zflat.square().sum(-1)
                        - 0.5 * Kz * math.log(2.0 * math.pi))
            # One complete stratified mixture per scramble.  This is
            #   (1/R) sum_r sum_m a_m (1/L) sum_l target/q,
            # and reduces to the former -log(R*M*L) expression for a_m=1/M.
            log_node_weight = (allocation[:, None, :, None].log()
                               - math.log(reps * per))
            log_node_weight = log_node_weight.expand(-1, reps, -1, per).reshape(B, -1)
            base = logprior - logq + log_node_weight
        ...
        self._last_qmc_layout = (reps, 1, x.shape[0] // reps)
        scaled = x.unsqueeze(0) * sd[:, 0, None, :]
        delta = (torch.einsum("bpk,bdk->bpd", scaled, Q[:, 0])
                 if Q.ndim == 4 else torch.matmul(scaled, Q.transpose(0, 1)))
    ...
    def _log_Z_size_multimode(self, ix, drop_empty, return_ess, return_size,
                              return_mode):
        """Size-stratified fixed-budget deterministic-mixture RQMC normaliser."""
        B = ix.B
        ...
        def _block(zc, bc):
            if return_size:
                ...
                # Detached node-size masses check d log Z/d(uniform b)=E[n] without
                # retaining a second autograd graph.
                return torch.logsumexp(joint, dim=1), node_log, joint.detach()
            node_log = log_f_sparse(self, zc, ix, C, drop_empty) + bc
        ...
        for lo in range(0, P, chunk):
            ...
            block_out = (torch.utils.checkpoint.checkpoint(
                _block, zc, bc, use_reentrant=False)
                if use_checkpoint else _block(zc, bc))
            if return_size:
                mass_c, node_c, size_c = block_out
                node_size_logs.append(size_c)
            else:
                mass_c, node_c = block_out
        ...
        if return_ess:
            ...
            reps, modes, per = self._last_qmc_layout
            if reps > 1 and P == reps * modes * per:
                nl4 = nl.view(B, reps, modes, per)
                rep_lz = torch.logsumexp(nl4, dim=(-1, -2)) + math.log(reps)
                self._last_qmc_logz_se = (
                    rep_lz.detach().std(dim=1, unbiased=True) / math.sqrt(reps))
                if per >= 4:
                    half_lz = (torch.logsumexp(
                        nl4[..., :per // 2].reshape(B, -1), dim=1) + math.log(2.0))
                    self._last_qmc_nested_logz_gap = (half_lz - lz).abs().detach()
                else:
                    self._last_qmc_nested_logz_gap = None
            else:
                self._last_qmc_logz_se = None
            ...
            self._last_qmc_nested_logz_gap = None
        if return_size:
            # Scalar log-Z agreement does not imply gradient agreement.  E[n] is the
            # derivative in the uniform-utility direction, so audit it under the same
            # independent scrambles and nested half/full rule.
            nsl = torch.cat(node_size_logs, dim=1)
            reps, modes, per = self._last_qmc_layout
            if reps > 1 and P == reps * modes * per:
                nsl5 = nsl.view(B, reps, modes, per, nsl.shape[-1])
                rep_mass = torch.logsumexp(nsl5, dim=(-3, -2)) + math.log(reps)
                n_axis = torch.arange(1, nsl.shape[-1] + 1, dtype=pn.dtype,
                                      device=pn.device)
                rep_en = (torch.softmax(rep_mass, dim=-1) * n_axis).sum(-1)
                self._last_qmc_en_se = (
                    rep_en.std(dim=1, unbiased=True) / math.sqrt(reps)).detach()
                if per >= 4:
                    half_mass = (torch.logsumexp(
                        nsl5[..., :per // 2, :].reshape(B, -1, nsl.shape[-1]), dim=1)
                                 + math.log(2.0))
                    half_en = (torch.softmax(half_mass, dim=-1) * n_axis).sum(-1)
                    full_en = (pn.detach() * n_axis).sum(-1)
                    self._last_qmc_nested_en_gap = (half_en - full_en).abs().detach()
                else:
                    self._last_qmc_nested_en_gap = None
            else:
                self._last_qmc_en_se = None
    ...
    def _log_Z_adaptive(self, ix, drop_empty, return_ess, return_size, return_mode,
        ...
        def _block(xc, wc):
            ...
            node_log = log_f_sparse(self, zc, ix, C, drop_empty) + base
            return torch.logsumexp(node_log, dim=1), node_log
    ...
    def log_Z_observed_size(self, ix, observed_n, return_ess=False, return_mode=False):
        """Direct sparse or RQMC log Z_n(x) for each trip's observed size.
        ...
        if getattr(self, "quad_a", None) is None:
            if getattr(self, "quad", None) is None:
                raise ValueError(
                    "observed-size normalizer requires a direct sparse or positive Sobol rule")
            # The fixed-size coefficient uses the identical Hubbard--Stratonovich
            # expectation as the full normalizer; only the final degree sum is replaced by
            # selecting the observed coefficient.  Signed direct quadrature is therefore
            # valid without a proposal, a mode solve, or importance weights.  Truncating
            # the polynomial at max(observed_n) is exact because higher coefficients cannot
            # contribute to any selected Z_n.
            nodes, weights = self.quad
            nodes = nodes.to(dtype=self.lam.dtype, device=self.lam.device)
            weights = weights.to(dtype=self.lam.dtype, device=self.lam.device)
            cache = sparse_prepare(self, ix, degree=max_n, nmax=max_n)
            z = nodes.unsqueeze(0).expand(ix.B, nodes.shape[0], self.Kz)
            terms = log_f_sparse(
                self, z, ix, cache, True, return_terms=True,
                nmax=max_n).permute(1, 0, 2)                 # [B,node,n]
            row = torch.arange(ix.B, device=self.lam.device)
            selected = terms[row, :, observed_n - 1]
            log_z, sign, log_condition = signed_log_integral(
                selected, weights, dim=1)
            self._last_quad_log_condition = log_condition.detach()
            if bool((sign != 1).any()) or not bool(torch.isfinite(log_z).all()):
                raise FloatingPointError(
                    "signed sparse quadrature produced a non-positive fixed-size "
                    "partition estimate; "
                    f"minimum sign={float(sign.min().detach()):.0f}, "
                    f"maximum log cancellation="
                    f"{float(log_condition.max().detach()):.6g}")
            out = [log_z]
            if return_ess:
                out.append(torch.ones(ix.B, dtype=log_z.dtype, device=log_z.device))
            if return_mode:
                out.append(torch.zeros(ix.B, self.Kz, dtype=log_z.dtype,
                                       device=log_z.device))
            return out[0] if len(out) == 1 else tuple(out)
    ...
    def sample(self, ix, n_draws=64, generator=None, mode_steps=1):
        ...
        # Stages 2--4 stay in log coordinates on the original complete support.  The old
        # rollout called log_f_ragged(return_parts=True), which formed
        # exp[-rho_c choose(r,2)] in linear space.  At R=120 that overflows once an
        # attractive category crosses rho_c=-0.0994, even though the version-4 log
        # probability is finite.  Run198 therefore trained with a valid log normaliser but
        # failed its sampler check at rho_c=-0.104.  Build the size law with the same fused
        # log kernel as likelihood, and use normalized/log dynamic programs below for the
        # two discrete reverse passes.  This is algebraically the same Corollary-3 chain;
        # it changes only its floating-point coordinates.
        C_sample = sparse_prepare(self, ix)
        lg_all = log_f_sparse(
            self, zsel.unsqueeze(1), ix, C_sample, True,
            return_terms=True)[0]                    # [B,n], index i is size i+1

        phi_i = self.phi[ix.item]
        slot_logw = (self.b_flat(ix) - 0.5 * phi_i.square().sum(-1)
                     + (zsel[ix.item_trip] * phi_i).sum(-1))
        slot_scale = seg_max(slot_logw.unsqueeze(0), ix.item_trip, B)[0]
        centred_logw = slot_logw - slot_scale[ix.item_trip]
        w_all = torch.exp(centred_logw)               # <= 1, safe item ESP coordinates

        sampled_n = []
        for b in range(B):
            row_lg = lg_all[b]
            if not bool(torch.isfinite(torch.logsumexp(row_lg, 0))):
                raise RuntimeError(f"non-finite log size law for sampler trip {b}")
            sampled_n.append(int(torch.multinomial(
                torch.softmax(row_lg, 0), 1, generator=generator)) + 1)
        sampled_max = max(sampled_n, default=1)
        # e_r of centred weights is non-negative and only degrees that can actually be
        # sampled are required.  Convert to log before adding the potentially large but
        # finite category attraction.
        row_esp = esp_bucketed(
            w_all.unsqueeze(0), ix.row_of, ix.n_rows, sampled_max,
            ix.row_size, ix.item_pos, native=bool(getattr(self, "_esp_native", False)))[0]
        r_axis = torch.arange(sampled_max + 1, dtype=w_all.dtype,
                              device=w_all.device)
        logG_all = torch.where(
            row_esp > 0, torch.log(row_esp),
            torch.full_like(row_esp, -float("inf")))
        logG_all = (logG_all
                    - self.rho_c[ix.row_cat].unsqueeze(-1)
                    * self.pair_feature(r_axis))
        out = []
        for b in range(B):
            rows = (ix.row_trip == b).nonzero().flatten().tolist()
            n = sampled_n[b]
            # --- level 3: log-prefix products over this trip's categories -----------
            polys = [logG_all[r_, :n + 1] for r_ in rows]
            pref = [torch.zeros(1, dtype=w_all.dtype, device=w_all.device)]
            for logGc in polys:
                previous = pref[-1]
                width = min(n, previous.shape[0] + logGc.shape[0] - 2) + 1
                nxt = torch.full((width,), -float("inf"), dtype=w_all.dtype,
                                  device=w_all.device)
                for r_take in range(min(logGc.shape[0], width)):
                    take = min(previous.shape[0], width - r_take)
                    if take > 0:
                        nxt[r_take:r_take + take] = torch.logaddexp(
                            nxt[r_take:r_take + take],
                            previous[:take] + logGc[r_take])
                pref.append(nxt)
            ...
            for c in range(len(polys) - 1, -1, -1):
                if left == 0:
                    break
                logGc, logP = polys[c], pref[c]
                hi = min(left, logGc.shape[0] - 1)
                cand_log = torch.stack([
                    logGc[r] + logP[left - r]
                    if (left - r) < logP.shape[0]
                    else torch.full((), -float("inf"), dtype=w_all.dtype,
                                    device=w_all.device)
                    for r in range(hi + 1)])
                if not bool(torch.isfinite(torch.logsumexp(cand_log, 0))):
                    raise RuntimeError(
                        f"non-finite category split for sampler trip {b}, size {n}")
                r_take = int(torch.multinomial(
                    torch.softmax(cand_log, 0), 1, generator=generator))
                if r_take:
                    # --- level 4: fixed-cardinality item subset in log coordinates ---
                    sel = (ix.row_of == rows[c]).nonzero().flatten()
                    lwc = centred_logw[sel]
                    logE = torch.full((len(lwc) + 1, r_take + 1), -float("inf"),
                                      dtype=lwc.dtype, device=lwc.device)
                    logE[0, 0] = 0.0
                    for k in range(1, len(lwc) + 1):
                        logE[k] = logE[k - 1]
                        logE[k, 1:] = torch.logaddexp(
                            logE[k - 1, 1:], lwc[k - 1] + logE[k - 1, :-1])
                    need = r_take
                    for k in range(len(lwc), 0, -1):
                        if need == 0:
                            break
                        log_probability = (lwc[k - 1] + logE[k - 1, need - 1]
                                           - logE[k, need])
                        probability = float(torch.exp(log_probability).clamp(0.0, 1.0))
                        if float(torch.rand(1, generator=generator)) < probability:
                            chosen.append(int(ix.item[sel[k - 1]]))
                            need -= 1
                    if need != 0:
                        raise RuntimeError(
                            f"fixed-size item sampler left {need} slots unfilled")
                left -= r_take
            if left != 0:
                raise RuntimeError(
                    f"category sampler left {left} of {n} slots unallocated")
    ...
    def pseudo_loglik(self, ix, line_item, line_trip, line_cat, B, line_ctx=None,
        ...
        absent_logp = -sp(allc)
        # The joint law has support 1 <= |S| <= nmax.  If the other coordinates already
        # contain nmax products, an absent coordinate is deterministically zero.  This is
        # a support constraint, not a numerical clipping of its finite-energy logit.
        absent_logp = torch.where(
            n_tot[ix.item_trip] >= self.nmax, torch.zeros_like(absent_logp), absent_logp)
        lp = torch.zeros(B, dtype=dt, device=dev).index_add_(
            0, ix.item_trip, absent_logp)
        ...
        wrong_absent_logp = -sp(wrong)
        wrong_absent_logp = torch.where(
            n_tot[line_trip] >= self.nmax,
            torch.zeros_like(wrong_absent_logp), wrong_absent_logp)
        present_logp = right - sp(right)
        # Removing the sole purchased item would create the excluded empty basket, hence
        # P(x_j=1 | x_-j=0)=1 exactly and its log contribution is zero.
        present_logp = torch.where(
            n_tot[line_trip] - one <= 0,
            torch.zeros_like(present_logp), present_logp)
        lp = lp.index_add_(0, line_trip, present_logp - wrong_absent_logp)
        return lp
