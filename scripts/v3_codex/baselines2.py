...
from the empirical size distribution.  Conditional on n, this is version 3's additive
family (phi = rho_c = 0).  It is NOT literally a nested joint model: the baseline is handed
a trip-independent empirical P(n), whereas version 3 induces a trip-dependent P(n|x)
...
def run(name, model, Bt, tr, va, a, te=None, **kw):
    ...
    order_gen = torch.Generator().manual_seed(0)
    t0 = time.time()
    for it in range(1, a.iters + 1):
        d = Bt.make(tr[rng.choice(len(tr), size=a.batch, replace=False)])
        step_kw = dict(kw)
        if isinstance(model, Shopper):
            step_kw["gen"] = order_gen
        loss = -model.loglik(d, **step_kw).mean()
        opt.zero_grad()
        loss.backward()
...
@torch.no_grad()
def ev(model, Bt, trips, chunk=32, seed=0, **kw):
    """Deterministic evaluation; SHOPPER receives a private ordering stream."""
    tot, nb, nl = 0.0, 0, 0
    ...
    for k in range(0, len(trips), chunk):
        ...
        call_kw = dict(kw)
        if isinstance(model, Shopper):
            call_kw["gen"] = order_gen
        tot += float(model.loglik(d, **call_kw).sum())
        nb += d["B"]
        nl += len(d["li"])
