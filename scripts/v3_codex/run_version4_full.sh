...
# Fresh production fit of the original version-4 joint law.  The trainer's guard rejects
# the command before optimization if the affinity partition, lineage, rank, catalogue,
# support, size law, or normalizer has drifted from the audited configuration.
cd "$(dirname "$0")/../.."
V3_AFFINITY=1 python -u scripts/v3/fit.py \
  ...
  2>&1 | tee out/v3_run155_v4_original_stochastic_full.log
